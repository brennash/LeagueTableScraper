...
create table 2015_IRL0_Fixtures 
(
  FixtureSID INT AUTO_INCREMENT, 
  LeagueCode VARCHAR(6) NOT NULL, 
  SeasonCode INT NOT NULL, 
  Date DATE, 
  ...
  AwayHT INT, 
  Venue VARCHAR(64), 
  PRIMARY KEY(FixtureSID, LeagueCode , SeasonCode)
);
...
CREATE PROCEDURE UpdateLeagueTable()
  ...
  CREATE TEMPORARY TABLE IF NOT EXISTS TempTeams AS 
  (SELECT DISTINCT(HomeTeam) FROM 2015_IRL0_Fixtures);
...
create table 2015_IRL0_Teams 
(
  TeamSID INT NOT NULL AUTO_INCREMENT, 
  TeamName VARCHAR(64), 
  TeamLogoURL VARCHAR(256), 
  HomeJerseyURL VARCHAR(256), 
  AwayJerseyURL VARCHAR(256), 
  Stadium VARCHAR(64), 
  Manager VARCHAR(64), 
  Honours VARCHAR(512), 
  PRIMARY KEY(TeamSID)
